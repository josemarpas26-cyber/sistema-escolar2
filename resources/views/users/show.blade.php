@extends('layouts.app')

@section('page-title', 'Detalhes do Utilizador')

@section('header-actions')
<a href="{{ route('users.edit', $user) }}" class="btn btn-primary">
    <i class="fas fa-edit mr-2"></i>
    Editar
</a>
@endsection

@section('content')

<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">

    <!-- Conteúdo Principal -->
    <div class="lg:col-span-2 space-y-6">

        <!-- Dados Pessoais -->
        <x-card title="Dados Pessoais" icon="fas fa-user">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">

                <div class="md:col-span-2">
                    <p class="text-sm text-gray-500 mb-1">Nome Completo</p>
                    <p class="font-semibold text-gray-900">{{ $user->name }}</p>
                </div>

                <div>
                    <p class="text-sm text-gray-500 mb-1">Número de Processo</p>
                    <p class="font-semibold text-gray-900">
                        {{ $user->numero_processo ?? '—' }}
                    </p>
                </div>

                <div>
                    <p class="text-sm text-gray-500 mb-1">Bilhete de Identidade (BI)</p>
                    <p class="font-semibold text-gray-900">
                        {{ $user->bi ?? '—' }}
                    </p>
                </div>

                <div>
                    <p class="text-sm text-gray-500 mb-1">Data de Nascimento</p>
                    <p class="font-semibold text-gray-900">
                        {{ $user->data_nascimento ? $user->data_nascimento->format('d/m/Y') : '—' }}
                    </p>
                </div>

                <div>
                    <p class="text-sm text-gray-500 mb-1">Género</p>
                    <p class="font-semibold text-gray-900">
                        {{ $user->genero === 'M' ? 'Masculino' : ($user->genero === 'F' ? 'Feminino' : '—') }}
                    </p>
                </div>

                <div>
                    <p class="text-sm text-gray-500 mb-1">Telefone</p>
                    <p class="font-semibold text-gray-900">{{ $user->telefone ?? '—' }}</p>
                </div>

                <div>
                    <p class="text-sm text-gray-500 mb-1">Email</p>
                    <p class="font-semibold text-gray-900">{{ $user->email ?? '—' }}</p>
                </div>

                @if($user->endereco)
                <div class="md:col-span-2">
                    <p class="text-sm text-gray-500 mb-1">Endereço</p>
                    <p class="font-semibold text-gray-900">{{ $user->endereco }}</p>
                </div>
                @endif

            </div>
        </x-card>

        <!-- Encarregado (alunos) -->
        @if($user->isAluno())
        <x-card title="Encarregado de Educação" icon="fas fa-user-friends">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-sm text-gray-500 mb-1">Nome</p>
                    <p class="font-semibold text-gray-900">{{ $user->nome_encarregado ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 mb-1">Contacto</p>
                    <p class="font-semibold text-gray-900">{{ $user->contacto_encarregado ?? '—' }}</p>
                </div>
            </div>
        </x-card>
        @endif

        <!-- Turmas (alunos) -->
        @if($user->isAluno() && $user->turmas->isNotEmpty())
        <x-card title="Histórico de Turmas" icon="fas fa-history">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Turma</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Curso</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Ano Letivo</th>
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase">Status</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Matrícula</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @foreach($user->turmas as $turma)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <a href="{{ route('turmas.show', $turma) }}"
                                   class="text-primary-600 hover:text-primary-900 font-medium">
                                    {{ $turma->nome_completo }}
                                </a>
                            </td>
                            <td class="px-6 py-4">{{ $turma->curso->nome }}</td>
                            <td class="px-6 py-4">{{ $turma->anoLetivo->nome }}</td>
                            <td class="px-6 py-4 text-center">
                                <x-badge type="{{ $turma->pivot->status === 'matriculado' ? 'success' : 'gray' }}">
                                    {{ ucfirst($turma->pivot->status) }}
                                </x-badge>
                            </td>
                            <td class="px-6 py-4">
                                {{ \Carbon\Carbon::parse($turma->pivot->data_matricula)->format('d/m/Y') }}
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </x-card>
        @endif

        <!-- Atribuições (professores) -->
        @if($user->isProfessor() && $user->atribuicoes->isNotEmpty())
        <x-card title="Atribuições de Turmas" icon="fas fa-chalkboard-teacher">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Turma</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Disciplina</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Ano Letivo</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        @foreach($user->atribuicoes as $atrib)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <a href="{{ route('turmas.show', $atrib->turma) }}"
                                   class="text-primary-600 hover:text-primary-900 font-medium">
                                    {{ $atrib->turma->nome_completo }}
                                </a>
                            </td>
                            <td class="px-6 py-4">{{ $atrib->disciplina->nome }}</td>
                            <td class="px-6 py-4">{{ $atrib->anoLetivo->nome }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </x-card>
        @endif

    </div>

    <!-- Sidebar -->
    <div class="space-y-6">

        <!-- Foto + Info -->
        <x-card title="Perfil" icon="fas fa-id-card">
            <div class="text-center">
                <img src="{{ $user->foto_perfil_url }}"
                     alt="{{ $user->name }}"
                     class="w-32 h-32 rounded-full mx-auto object-cover border-4 border-primary-200 mb-4">
                <h3 class="font-bold text-lg text-gray-900 mb-1">{{ $user->name }}</h3>
                <x-badge type="primary" class="text-sm">{{ $user->role->display_name }}</x-badge>

                <div class="mt-4 pt-4 border-t border-gray-200 text-left space-y-2 text-sm">
                    <div class="flex justify-between">
                        <span class="text-gray-500">Status:</span>
                        <x-badge type="{{ $user->ativo ? 'success' : 'danger' }}">
                            {{ $user->ativo ? 'Ativo' : 'Inativo' }}
                        </x-badge>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Cadastro:</span>
                        <span class="font-medium">{{ $user->created_at->format('d/m/Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Login por:</span>
                        <span class="font-medium text-xs text-right">
                            @if($user->email && $user->numero_processo)
                                Email ou Nº Processo
                            @elseif($user->email)
                                Email
                            @elseif($user->numero_processo)
                                Nº Processo
                            @else
                                —
                            @endif
                        </span>
                    </div>
                </div>
            </div>
        </x-card>

        <!-- Ações -->
        <x-card title="Ações" icon="fas fa-bolt">
            <div class="space-y-2">

                <a href="{{ route('users.edit', $user) }}"
                   class="w-full inline-flex items-center justify-center gap-2
                          px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600
                          hover:from-indigo-700 hover:to-purple-700 text-white font-semibold shadow-md
                          transition-all hover:scale-[1.02]">
                    <i class="fas fa-edit text-sm"></i>
                    Editar
                </a>

                @if($user->isAluno())
                <a href="{{ route('relatorios.boletim', $user) }}"
                   class="w-full inline-flex items-center justify-center gap-2
                          px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700
                          text-white font-semibold shadow-sm transition-all hover:scale-[1.02]">
                    <i class="fas fa-file-pdf text-sm"></i>
                    Ver Boletim
                </a>
                @endif

                <form method="POST" action="{{ route('users.toggle-status', $user) }}">
                    @csrf
                    <button type="submit"
                            class="w-full inline-flex items-center justify-center gap-2
                                   px-4 py-2.5 rounded-xl font-semibold text-white shadow-sm
                                   transition-all hover:scale-[1.02]
                                   {{ $user->ativo ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-green-600 hover:bg-green-700' }}">
                        <i class="fas fa-{{ $user->ativo ? 'ban' : 'check' }} text-sm"></i>
                        {{ $user->ativo ? 'Desativar' : 'Ativar' }}
                    </button>
                </form>

                @if($user->id !== auth()->id())
                <button onclick="confirmDelete('delete-form', 'Deseja eliminar {{ $user->name }}?')"
                        class="btn btn-danger w-full">
                    <i class="fas fa-trash mr-2"></i>
                    Eliminar Utilizador
                </button>
                <form id="delete-form" method="POST" action="{{ route('users.destroy', $user) }}" class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
                @endif

            </div>
        </x-card>

    </div>

</div>

@endsection