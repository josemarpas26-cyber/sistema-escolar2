<x-card title="Ranking de Alunos" icon="fas fa-trophy" class="mb-8">
    <form method="GET" action="{{ route('dashboard') }}" class="grid grid-cols-1 md:grid-cols-5 gap-3 mb-4">
        <div>
            <label class="block text-xs font-semibold text-gray-600 mb-1">Pesquisar aluno</label>
            <input type="text" name="q" value="{{ $filtros_ativos['q'] ?? '' }}" class="w-full rounded-lg border-gray-300 text-sm" placeholder="Nome do aluno">
        </div>
            ...
            @foreach($ranking_alunos as $index => $alunoRanking)
                <div class="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-3 py-2">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-[11px] text-gray-500">{{ $alunoRanking->total_notas }} notas</p>
                    </div>
                </div>
            @endforeach
        </div>
